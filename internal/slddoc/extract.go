...
func Extract(raw []byte, source string, voltageHints map[string]string) (*Diagram, Report, error) {
	...
	addElement := func(el Element, ports []Point) {
		colors = append(colors, el.Voltage)
		idx := len(d.Elements)
	...
	for _, n := range root.Children {
		...
		case "21", "22", "23", "28":
			...
			colors = append(colors, c.Voltage)
		...
		case "7":
			...
			addElement(el, []Point{{X: el.X, Y: el.Y}})
		...
		case "41", "42", "43", "71", "162", "49", "33", "34", "35", "203", "388":
			el, ports, err := parseTwoPortDevice(n, twoPortShapes[dt], dt)
			if err != nil {
				report.Failed = append(report.Failed, n.attr("id"))
				continue
			}
	...
	for i := range d.Elements {
		d.Elements[i].Voltage = colorToID[normalizeColor(d.Elements[i].Voltage)]
	}
	for i := range d.Connectors {
		d.Connectors[i].Voltage = colorToID[normalizeColor(d.Connectors[i].Voltage)]
	}
