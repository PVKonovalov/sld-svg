...
func TestBuildTopology_ExactCoincidenceAndBusTap(t *testing.T) {
	d := &Diagram{
		Elements: []Element{
			{ID: "bus", Class: ClassBusBarSection, Points: []Point{{800, 240}, {1300, 240}}},
			{ID: "brk", Ports: []Port{{Name: "1"}, {Name: "2"}}},
		},
		Connectors: []Connector{
			{ID: "w1", Points: []Point{{900, 240}, {900, 270}}}, // taps the bus mid-span
		},
	}
	...
	if d.Connectors[0].From == "" || d.Connectors[0].To == "" {
		t.Fatalf("connector endpoints not resolved: %+v", d.Connectors[0])
	}
	if d.Elements[1].Ports[0].Node != d.Connectors[0].To {
		t.Errorf("breaker port 1 (%s) should share a node with the wire's far end (%s)",
	...
	if d.Connectors[0].From != busNodeID {
		t.Errorf("wire tapping the bus mid-span should resolve to the bus's node %s, got %s", busNodeID, d.Connectors[0].From)
	}
}
...
func TestBuildTopology_SnapsSmallGaps(t *testing.T) {
	d := &Diagram{
		Elements: []Element{
			{ID: "t", Ports: []Port{{Name: "1"}}},
		},
		Connectors: []Connector{
			{ID: "w", Points: []Point{{900, 550}, {900, 570}}},
		},
	}
	bindings := []portBinding{
	...
	if d.Elements[0].Ports[0].Node != d.Connectors[0].From {
		t.Errorf("a port within snapTolerance of a wire end should merge with it: port node %s, wire from %s",
